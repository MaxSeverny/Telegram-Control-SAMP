...
2.0.0 | 27.08  
    + Добавил чекер;
    + Рзменил размер основного imgui окна;
    + Удалил кнопку Домой;
...
4.2.0 | 17.03
    ...
    + Рзменил функцию Мои Евенты;
    * Фикс с логом обновлений;
    * Фикс вкладки Команды;
    *Рзменил дерикторию сохрание настроек
...
function main() -- #main()
    ...
    if not doesFileExist('moonloader\\config\\Telegram Control SAMP\\MyEvents.txt') then 
        e = io.open('moonloader\\config\\Telegram Control SAMP\\MyEvents.txt', 'w')
        e:write('')
        e:close()    
...
function onReceivePacket(id, bs)
    lua_thread.create(function()
        if id == 32 or id == 33 or id == 36 or id == 37 and disconnected == false --[[шоб не флудило~]] then
            ...
            disconnected = true 
            sendTelegramNotification('[SAMP]:\nСоеденение с сервером было прервано!')
            --LoadingScript_imgui.v = true 
            --wait(15000)
        end
    end)
...
function imgui.OnDrawFrame() -----------------------------------------------------------------------
    ...
    if imgui_window.v then
        ...
        imgui.Begin(fa.ICON_TELEGRAM .. ' Telegram Control SAMP | ' .. __version__, imgui_window, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoFocusOnAppearing)
            imgui.BeginChild('#a', imgui.ImVec2(220, 465), true)
                if imgui.ToggleButton(u8'Включить Уведомления ' .. fa.ICON_COMMENTS, ToggleButton.Bot) then 
                    PushMessage(ToggleButton.Bot.v and u8'Уведомления на ваш Телеграмм-Аккаунт будут присылатся ' .. fa.ICON_VOLUME_UP or u8'Слышишь?Тишина ' .. fa.ICON_VOLUME_OFF .. u8' ..\nУведомления на ваш телеграмм-аккаунт не будут присылатся\nТихий режим ' .. fa.ICON_BED .. 'zzZZ')
                    if ToggleButton.Bot.v then 
                        sendTelegramNotification('Уведомления - Включены!')
                    end
                end
                ...
                if not doesFileExist('moonloader\\config\\Telegram Control SAMP\\AutoRebootTelegramControlSAMP.lua') then 
                    ...
                    imgui.FontSize(0.9)
                    imgui.TextDisabled(u8'файл закреплен в теме на БХ')
                    imgui.FontSize(1.0)
                else
                ...
                if imgui.ButtonHex(fa.ICON_FLOPPY_O .. u8' Сохранить ВСЁ Настройки | ' .. fa.ICON_QUESTION_CIRCLE, 0x00fbff00, imgui.ImVec2(-1, 25)) then save() end       
                if imgui.Button(fa.ICON_BARS .. u8' Лог Обновлений..', imgui.ImVec2(-1, 30)) then imgui.OpenPopup('Log Updates') end   
                imgui.Separator()              
                ...
                if imgui.BeginPopupModal('Log Updates', nil, --[[imgui.ImVec2(500, 500)]] imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoTitleBar) then 
                            if imgui.ButtonHex(u8'Закрыть', 0x00FF0000, imgui.ImVec2(-1, 30), 5) then imgui.CloseCurrentPopup() end
                        imgui.Separator()
                        imgui.Text(u8(UpdateLog1))
                        imgui.Text(u8(UpdateLog2))
                        imgui.Separator()
                    imgui.EndPopup()
                end                 
                ...
                imgui.Text(u8('Автор:Vespan'))
                imgui.Text(u8'Тема на Blast.hk:') imgui.Link('https://www.blast.hk/threads/62811/')   

                imgui.Separator()

                imgui.CenterText(u8'Я вырезал АвтоРеконект!\nРспользуйте сторонний..\n..АвтоРеконект!')
        ...
        imgui.BeginChild('#okno', imgui.ImVec2(525, 450))
            if ImguiPage == 0 then ----------------------------------------------------------------
                ...
                if imgui.InputText(fa.ICON_TERMINAL .. u8' Открыть imgui окно', Input.Cmd) then     
                ...
                if imgui.Button(u8'Команды По Умолчанию##1', imgui.ImVec2(-1, 25)) then 
                    Input.Cmd_Send.v = '/send'
                    Input.Cmd_Chat.v = '/chat'
                    Input.Cmd_Quit.v = '/q'
                    Input.Cmd_FakeAFK.v = '/fakeafk'
                    Input.Cmd_AntiAFK.v = '/antiafk'
                    Input.Cmd_Time.v = '/time'
                    Input.Cmd_Stats.v = '/stats'
                    Input.Cmd_Dialog_Input.v = '/d.input'
                    Input.Cmd_Dialog_List.v = '/d.list'
                    Input.Cmd_Dialog_Close.v = '/d.close'
            ...
            elseif ImguiPage == 2 then -----#BOT-----------------------------------------------------------       
                if imgui.Button(u8'Показать##chat_id') then ShowChatId = not ShowChatId end
                    ...
                    if #Input.ChatId.v > 6 then
                        imgui.SameLine()
                        if imgui.Button(u8'Сохранить##chatid') then 
                    ...
                    imgui.SameLine()
                    if #Input.Token.v > 10 then 
                        if imgui.Button(u8'Сохранить##token') then 
            ...
            elseif ImguiPage == 3 then ---------#BOTSETTINGS #SETTINGSBOT-------------------------------------------------------
                ...
                imgui.Separator()
                    ...
                    imgui.InputTextMultiline(u8'##MyEvents', Input.MyEvents, imgui.ImVec2(250, 150)) 
                    imgui.SameLine()
                    if imgui.Button(u8'Сохранить Текст', imgui.ImVec2(135, 150)) then 
                        e = io.open('moonloader\\config\\Telegram Control SAMP\\MyEvents.txt', 'w')
                        e:write(Input.MyEvents.v)
                        e:close()
                    end                    
                    imgui.TextQuestion(u8'Что вы напишите,скрипт будет искать в чате\nКогда найдёт текст,то пришлет вам в телеграмм-чат полную строку из чата\nНеобходимо писать слова через Ентер!Пример:\n\nСобеседование\nСтрой\nВы тут?\n(оставьте пустьм поле,что бы не работала функция)')
                    if imgui.Button(u8'Загрузить##MyEvents') then 
                        if doesFileExist('moonloader\\config\\Telegram Control SAMP\\MyEvents.txt') then 
                            for line in io.lines('moonloader\\config\\Telegram Control SAMP\\MyEvents.txt') do
                                Input.MyEvents.v = Input.MyEvents.v .. line .. '\n'
                            end                             
                        end
                    end
                        --[[if Input.CheckerAdd.v ~= '' and doesFileExist('moonloader\\config\\Telegram Control SAMP\\Checker.txt') then 
                    ...
                    imgui.ToggleButton('TimeStamp', ToggleButton.TimeStamp)
                    imgui.ToggleButton(u8'3 Раза прислать сообщение когда происходит  событие', ToggleButton.NotificationFlood) 

                imgui.Separator()
                imgui.ToggleButton(u8'Чекер Ргроков:', ToggleButton.Checker)
                if ToggleButton.Checker.v then 
                    --imgui.Text(u8('Ргрок зайдёт на сервер.'))
    ...
    if PushMessage_imgui.v then 
        imgui.PushStyleVar(imgui.StyleVar.Alpha, 0.7)     
        imgui.SetNextWindowPos(imgui.ImVec2(sw / 2.3, sh / 1.2), imgui.Cond.FirstUseEver, imgui.ImVec2(0.5, 0.5))
        imgui.Begin(u8'##notf', imgui_window, imgui.WindowFlags.AlwaysAutoResize + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.NoMove)
        imgui.ShowCursor = imgui_window.v
        imgui.SetWindowFontScale(1.3)
            imgui.CenterText((fa.ICON_TELEGRAM .. ' Telegram Control SAMP'))
        imgui.SetWindowFontScale(1.0)
        imgui.Separator()
        imgui.CenterText(nText)
    ...
    if LoadingScript_imgui.v then
        ...
        if isKeyJustPressed(VK_Y) and not sampIsDialogActive() and not sampIsChatInputActive() and not isSampfuncsConsoleActive() then 
            LoadingScript_imgui.v = false 

                if not ToggleButton.Bot.v then
                    PushMessage(u8'Успешно загружен!\n\n            ' .. fa.ICON_EXCLAMATION .. u8'Сообщение выключены!\n' .. fa.ICON_VOLUME_OFF..u8'Открыть имгуи-окно /'..Config.Script.Cmd)
                else
        ...
        imgui.Text('') imgui.SameLine(95) imgui.Spinner(15, 2)
                            ...
                            -- old elseif text:match(~~~) then 
                                elseif text == Input.Cmd_Stats.v then -- #GETSTATS #STATS                                     
                                    ...
                                    local x, y, z = getCharCoordinates(PLAYER_PED)
                                        ...
                                        local function CheckArizona() --#CheckArizona
                                    ...
                                    sendTelegramNotification(string.format('[SAMP | Get Player Info]:\n>%s[ %d ]<\n%s - %s\n\nБроня:%d+ | ХП:%d+\nПинг:%d+ | Уровень:%d+' 
                                        .. '\nГолод(ARZ):%s | Деньги:%d+\nАнимация:%s[%d+]\nВ руках:%s\n\nВ Зоне-Стрима:%d+\nОнлайн Сервера:%d+\n%s', ---------------[[]]
                                        sampGetPlayerNickname(id), id, city(getCityPlayerIsIn(PLAYER_PED)), calculateZone(math.floor(x), math.floor(y), math.floor(z)),
                                        sampGetPlayerArmor(id), sampGetPlayerHealth(id), sampGetPlayerPing(id), sampGetPlayerScore(id), CheckArizona(), getPlayerMoney(player), animfile, animid,
                                        getweaponname(getCurrentCharWeapon(PLAYER_PED)), sampGetPlayerCount(true), sampGetPlayerCount(true), CheckStatusConnect()--[[sampGetPlayerCount(false)]] ) )
                                elseif text:match(Input.Cmd_Send.v) then  -- #SEND
                                    local send = text:gsub(Input.Cmd_Send.v .. ' ','')
                                    if #send > 0 then 
                                        -- sendTelegramNotification('(?) Введи /send [text] что бы отправить сообщение в чат')
                                        sampSendChat(send)
                                        wait(50)
                                    ...
                                    elseif #send == 0 then
                                        sendTelegramNotification('(?) Введи ' .. Input.Cmd_Send.v .. ' [text] что бы отправить сообщение в чат')
                                    end
                                ...
                                elseif text == '/help' then 
                                    ...
                                    .. Input.Cmd_Time.v .. ' - Посмореть сколько время я отыграл БЕЗ AFK/С AFK\n'
                                    .. Input.Cmd_Quit.v .. ' - Выход из gta_sa.exe\n'
                                ...
                                elseif text == Input.Cmd_Dialog_List.v then 
                                    if sampIsDialogActive() then 
                                       local list = text:gsub(Input.Cmd_Dialog_List.v .. ' ','',1)
                                       if #list > 0 then 
                                            sampSendDialogResponse(idDialog, 1, list - 1, -1)
                                            sendTelegramNotification('[SAMP | Dialog]:\nВы Выбрали Пункт: ' .. list)
                                        else
                                            sendTelegramNotification('(?) ' .. Input.Cmd_Dialog_List.v .. ' [пункт(НАЧРНАТЬ ОТ 0)] - Что бы выбрать пункт')
                                        end
                                    else
                                ...
                                elseif text:match(Input.Cmd_Dialog_Input.v) then
                                --elseif text == Input.Cmd_Dialog_Input.v then 
                                    if sampIsDialogActive() then 
                                       local input = text:gsub(Input.Cmd_Dialog_Input.v .. ' ','',1)
                                       if #input > 1 then 
                                            sampSendDialogResponse(idDialog, 1, -1, input)
...
function sampev.onServerMessage(color, text)
    ...
    for k,v in pairs(tbl_MyEvents) do
        if text:find(u8:decode(v)) then
            sendTelegramNotification('[SAMP | MyEvents]:\n' .. text)
        end
    end
...
function AutoHeal()
    while true do wait(0)
        ...
        if ToggleButton.AutoHeal.v then -- #AUTOHEAL #AUTOHP
            if not getCharHealth(PLAYER_PED) == 0 and getCharHealth(PLAYER_PED) < ImInt.HP.v then
...
function PushMessage(text)
    lua_thread.create(function()
        nText = nText .. text .. '\n\n'
        PushMessage_imgui.v = true 
    wait(10000)
        PushMessage_imgui.v = false
        nText = ''
    end)
end
