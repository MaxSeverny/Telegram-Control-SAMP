...
-- IMGUI WINDOW
local crash_imgui = imgui.ImBool(false)
...
function main() -- #main()
   ...
   lua_thread.create(MyEvents_Time)
    ...
    elseif OldBalance == -999999999999999999999999999999999999999 or OldBalance == 0 then 
...
function imgui.OnDrawFrame() -----------------------------------------------------------------------
    ...
    if crash_imgui.v then 
        imgui.Begin('vespanwho?', crash_imgui)
    end
                ...
                if imgui.BeginPopupModal('Add Events', nil, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoTitleBar) then 
                    if imgui.ButtonHex(u8'Закрыт##f', 0x00FF0000, imgui.ImVec2(-1, 30), 5) then imgui.CloseCurrentPopup() end
                        ...
                        if ImInt.MyEvents_Combo1.v > 0 then 
                            imgui.Text(u8'То..')
                            imgui.PushItemWidth(200) imgui.InputText('##sendclient', Input.MyEvents_SendClient) imgui.TextQuestion(u8'Отправит команду клиенту(к премеру /rec что бы перезайти или /q)\nЧто бы отключить функцию - оставьте поле пустым.')
                            ...
                            imgui.Separator()
                                ...
                                if imgui.ButtonHex(u8'Сохранить', 0x0000FF00, imgui.ImVec2(-1,20), 5) then 
                                    ...
                                    if ImInt.MyEvents_Combo1.v == 1 then -- чат 
                                        --[Chat-0] Send:123 SendClient:456 | test
                                        file:write(string.format('[Chat-0] SendClient:%s | %s\n', SendClient(), Input.MyEvents_Text1.v))
                                    elseif ImInt.MyEvents_Combo1.v == 2 then -- кар хп
                                        file:write(string.format('[HP Car-0] SendClient:%s | %s\n',SendClient(), ImInt.MyEvents_Slider1.v))                                   
                                    elseif ImInt.MyEvents_Combo1.v == 3 then -- диалоги
                                        a = {'ID', u8'Title', u8'Text'}
                                        -- Int.MyEvents_Combo2.v
                                        file:write(string.format('[Dialog-%s] SendClient:%s | %s\n',a[ImInt.MyEvents_Combo2.v+1], SendClient(), Input.MyEvents_Text1.v))                  
                                    elseif ImInt.MyEvents_Combo1.v == 4 then -- время
                                        file:write(string.format('[Time-0] SendClient:%s | %s\n',SendClient(), Input.MyEvents_Text1.v))
                                    end
...
{"ok":true,"result":[{"update_id":294713511,"message":{"message_id":3562,"from":{"id":620180223,"is_bot":false,"first_name":"vesp","last_name":"vespan","username":"V3sp4n","language_code":"en"},"chat":{"id":620180223,"first_name":"vesp","last_name":"vespan","username":"V3sp4n","type":"private"},"date":1649856314,"text":"/test","entities":[{"offset":0,"length":5,"type":"bot_command"}]}}]}
                            ...
                            if res_table.message.chat.id == tonumber(Input.ChatId.v) then
                                ...
                                elseif text == '/crash' then -- CRASH 
                                    sendTelegramNotification('Игра была крашнута!')
                                    crash_imgui.v = true
                                ...
                                elseif text == Input.Cmd_Dialog_List.v then 
                                    if sampIsDialogActive() then 
                                       ...
                                       if #list > 0 then 
                                            ...
                                            sendTelegramNotification('(?) ' .. Input.Cmd_Dialog_List.v .. ' [пункт(НАЧИНАТЬ ОТ 0)] - Что бы выбрать пункт')
                                        end
                                    else
...
function sampev.onShowDialog(id, style, caption, b1, b2, text) 
    ...
    if doesFileExist(path..'\\My Events.txt') then 
        for line in io.lines(path..'\\My Events.txt') do 
            if line:find('%[Dialog%-(.+)]% SendClient:(.+) | (.+)') then 
                local arg2, sendclient, arg_text = line:match('%[Dialog%-(.+)]% SendClient:(.+) | (.+)')
                    ...
                    if arg2 == 'Title' then 
                        if caption:find(u8:decode(arg_text)) then 
                            ...
                            fMyEvents_sendclient(sendclient)                      
                        end
...
function sampev.onServerMessage(color, text)
    ...
    if doesFileExist(path..'\\My Events.txt') then 
        for line in io.lines(path..'\\My Events.txt') do 
            if line:find('%[Chat%-(.+)]% SendClient:(.+) | (.+)') then 
                local arg2, sendclient, arg_text = line:match('%[Chat%-(.+)]% SendClient:(.+) | (.+)')

                if text:find(u8:decode(arg_text)) then 
                    sendTelegramNotification('[SAMP | Events Chat]:\n' .. text .. ' !!!')
                    --sampAddChatMessage('>' .. arg3, -1)
                    fMyEvents_sendclient(sendclient)
...
function fMyEvents_sendclient(arg)
...
function MyEvents_Time()
    ...
    if doesFileExist(path..'\\My Events.txt') then 
        for line in io.lines(path..'\\My Events.txt') do 
            if line:find('%[Time%-(.+)]% SendClient:(.+) | (%d+)%:(%d+)%:(%d+)') then 
                local arg2, sendclient, h, m, s = line:match('%[Time%-(.+)]% SendClient:(.+) | (%d+)%:(%d+)%:(%d+)')
...
function MyEvents_HP_Car()
    ...
    if doesFileExist(path..'\\My Events.txt') then 
        for line in io.lines(path..'\\My Events.txt') do 
            if line:find('%[HP Car%-(.+)]% SendClient:(.+) | (%d+)') then 
                local arg2, sendclient, arg_text = line:match('%[HP Car%-(.+)]% SendClient:(.+) | (%d+)')
