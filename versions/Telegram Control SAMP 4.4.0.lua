...
local ImInt = {
    ...
    MyEvents_RadioButton1 = imgui.ImInt(0)
...
local ToggleButton = {
    ...
    MyEvents_Send = imgui.ImBool(false),
    MyEvents_SendClient = imgui.ImBool(false),
...
function main() -- #main()
   ...
   -- lua_thread.create(DoPayDaya)
    ...
    sampfuncsRegisterConsoleCommand('2', function() 
        sampAddChatMessage(''..Raw.version, -1)
end)

    sampfuncsRegisterConsoleCommand('2', function() 
    if doesFileExist(path..'\\My Events.txt') then 
        for line in io.lines(path..'\\My Events.txt') do 
            if line:find('%[Time-(.+)]% (%d+) | (.+)') then 
                local arg2, arg3, arg_text = line:match('%[Time-(.+)]% (%d+) | (.+)')
                print(arg_text)
            else
                print('0')
            end
        ...
        imgui.Begin(fa.ICON_TELEGRAM .. ' Telegram Control SAMP | ' .. __version__ .. '' .. GetTextUpdate(), imgui_window, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoFocusOnAppearing)
            imgui.BeginChild('#a', imgui.ImVec2(220, 465), true)
                ...
                if not doesFileExist('moonloader\\config\\Telegram Control SAMP\\AutoRebootTelegramControlSAMP.lua') then 
                    imgui.Separator()
                    --imgui.FontSize(0.7) imgui.TextColored(imgui.ImVec4(1,0,0,1), u8'У вас нет AutoRebootTelegramControlSAMP.lua')
                ...
                if imgui.Button(u8'хз чо тут.', imgui.ImVec2(-1,30)) then ImguiPage = 0 end
                imgui.Separator()              
...
То этот скрипт перезагрузит его]]))
                ...
                imgui.CenterText(u8'Я вырезал АвтоРеконект!\nИспользуйте сторонний..\n..АвтоРеконект!')

                --request = http.request("https://raw.githubusercontent.com/Vespan/Telegram-Control-SAMP/main/who%3F")
                --raw = split(request, "~")        
                --imgui.Text()
        ...
        imgui.BeginChild('#okno', imgui.ImVec2(525, 450))
            if ImguiPage == 0 then ----------------------------------------------------------------
                ...
                imgui.CenterText(u8(Raw.info))
                imgui.CenterText(u8(Raw.other))
            ...
            elseif ImguiPage == 4 then --------- #EVENTS #MYEVENTS ------------------------------
                ...
                imgui.BeginChild('##MyEvents', imgui.ImVec2(500, 350), true)
                ...
                if imgui.ButtonHex(u8'Очистить', 0x00FF629b, imgui.ImVec2(-1,30)) then imgui.OpenPopup('clear events?') end
                if imgui.ButtonHex(u8'Добавить', 0x006FF253, imgui.ImVec2(-1,50) ) then imgui.OpenPopup('Add Events') end 
                ---------------------------------------------------------------------------------------------------------------------
                ...
                if imgui.BeginPopupModal('Add Events', nil, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoTitleBar) then 
                    if imgui.ButtonHex(u8'Закрыт##f', 0x00FF0000, imgui.ImVec2(-1, 30), 5) then imgui.CloseCurrentPopup() end
                        imgui.Separator()
                        ...
                        elseif ImInt.MyEvents_Combo1.v == 4 then -- время os.date
                            ...
                            imgui.TextDisabled(u8'Формать времени указывать ЧЧ:ММ:СС(~12:30:00)')

                            hh,mm,ss = Input.MyEvents_Text1.v:match('(%d)(%d)%:(%d)(%d)%:(%d)(%d)')
                            if #Input.MyEvents_Text1.v > 7 then 
                                if not hh and not mm and not ss then
                                    imgui.Text(u8'Не правильный формат времени!')
                                end
                            end
                        ...
                        if ImInt.MyEvents_Combo1.v > 0 then 
                            imgui.Text(u8'То..')
                            imgui.RadioButton(u8'Оповестить меня', ImInt.MyEvents_RadioButton1, 1)
                            imgui.RadioButton(u8'Выйти из игры', ImInt.MyEvents_RadioButton1, 2) imgui.SameLine() imgui.TextDisabled(u8'и оповестить')
                            -- imgui.ToggleButton(u8'Написать в чат(to server)', ToggleButton.MyEvents_Send)
                            -- imgui.ToggleButton(u8'Написать в чат(to client)', ToggleButton.MyEvents_SendClient) 
                            -- потом добавлю возможность написать серверу/клиенту после событие.
                        end                        

                        if ImInt.MyEvents_Combo1.v > 0 and ImInt.MyEvents_RadioButton1.v > 0 then 

                            imgui.Separator()

                                if imgui.ButtonHex(u8'Сохранить', 0x0000FF00, imgui.ImVec2(-1,20), 5) then 
                                    ...
                                    if ImInt.MyEvents_Combo1.v == 1 then -- чат 
                                        file:write(string.format('[Chat-0] %d | %s\n',ImInt.MyEvents_RadioButton1.v, Input.MyEvents_Text1.v))
                                    elseif ImInt.MyEvents_Combo1.v == 2 then -- кар хп
                                        file:write(string.format('[HP Car-0] %d | %d\n',ImInt.MyEvents_RadioButton1.v, ImInt.MyEvents_Slider1.v))                                   
                                    elseif ImInt.MyEvents_Combo1.v == 3 then -- диалоги
                                        a = {'ID', u8'Title', u8'Text'}
                                        -- Int.MyEvents_Combo2.v
                                        file:write(string.format('[Dialog-%s] %d | %s\n',a[ImInt.MyEvents_Combo2.v+1], ImInt.MyEvents_RadioButton1.v, Input.MyEvents_Text1.v))                  
                                    elseif ImInt.MyEvents_Combo1.v == 4 then -- время
                                        file:write(string.format('[Time-0] %d | %s\n',ImInt.MyEvents_RadioButton1.v, Input.MyEvents_Text1.v))
                                    end
                                    ...
                                    ImInt.MyEvents_RadioButton1.v = 0

                                    ToggleButton.MyEvents_Send.v = false 
                                    ToggleButton.MyEvents_SendClient.v = false
                                ...
                                elseif text == Input.Cmd_Stats.v then -- #GETSTATS #STATS                                     
                                    ...
                                    local x, y, z = getCharCoordinates(PLAYER_PED)
                                        ...
                                        local function fNewBalance()
                                            if NewBalance > 0 and not NewBalance == OldBalance then 
...
function sampev.onShowDialog(id, style, caption, b1, b2, text) 
    ...
    if doesFileExist(path..'\\My Events.txt') then 
        for line in io.lines(path..'\\My Events.txt') do 
            if line:find('%[(.+)%-(.+)]% (%d+) | (.+)') then 
                local arg1, arg2, arg3, arg_text = line:match('%[(.+)%-(.+)]% (%d+) | (.+)')

                if arg1 == 'Dialog' then 

                    if arg2 == 'Title' then 
                        if caption:find(u8:decode(arg_text)) then 
                    ...
                    elseif arg2 == 'Text' then 
                        if text:find(u8:decode(arg_text)) then 
...
function sampev.onServerMessage(color, text)
    ...
    if doesFileExist(path..'\\My Events.txt') then 
        for line in io.lines(path..'\\My Events.txt') do 
            if line:find('%[Chat%-(.+)]% (%d+) | (.+)') then 
                local arg2, arg3, arg_text = line:match('%[Chat%-(.+)]% (%d+) | (.+)')
...
function fMyEvents_exit(arg)

    if arg == '2' then -- выход
        sendTelegramNotification('[SAMP | Events]:\nВыхожу из игры..(/q)')
        sampProcessChatInput('/q')
    elseif arg == '3' then 
        sampSendChat('')
    elseif arg == '4' then
...
function MyEvents_Time()
    ...
    if doesFileExist(path..'\\My Events.txt') then 
        for line in io.lines(path..'\\My Events.txt') do 
            if line:find('%[Time%-(.+)]% (%d+) | (.+)') then 
                local arg2, arg3, h, m, s = line:match('%[Time%-(.+)]% (%d+) | (%d+)%:(%d+)%:(%d+)')
...
function MyEvents_HP_Car()
    ...
    if doesFileExist(path..'\\My Events.txt') then 
        for line in io.lines(path..'\\My Events.txt') do 
            if line:find('%[HP Car%-(.+)]% (%d+) | (.+)') then 
                local arg2, arg3, arg_text = line:match('%[HP Car%-(.+)]% (%d+) | (.+)')
